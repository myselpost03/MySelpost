...
const TelegramPost = ({ channel, postId, onSeen }) => {
  ...
  return (
    <div style={{ position: 'relative', marginBottom: '25px', minHeight: '150px' }}>
      <div
        style={{
          position: 'absolute',
          inset: 0,
      ...
      />
      {/* Container must be empty for the script to inject properly */}
...
export default function MemeFeed() {
    ...
    if (!seenPosts.includes(id)) {
      ...
      if (isTelegram && newCount % 15 === 0) {
        ...
        const result = await triggerAd({
          ...
          networks: [
            primaryNetwork,
            ...INTERSTITIAL_ORDER.filter((n) => n !== primaryNetwork),
          ],
        });
  ...
  // --- Updated Logic inside MemeFeed ---

// 1. Simplify the loadMore function
const loadMore = useCallback(() => {
  if (isLoading) return;

  setIsLoading(true);

  // Always get a fresh list of what hasn't been seen
  const currentUnseen = getUnseenPosts();
  
  // Take the very first 6 posts from the current pool of unseen posts
  const nextPosts = currentUnseen.slice(0, 6);

  setTimeout(() => {
    setPostIds((prev) => {
      // Filter nextPosts to ensure they aren't already in the current view
      const uniqueNext = nextPosts.filter(id => !prev.includes(id));
      return [...prev, ...uniqueNext];
    });
    setIsLoading(false);
  }, 300);
}, [isLoading, seenPosts]); // Dependencies updated

// 2. Adjust initialization
const [postIds, setPostIds] = useState(() => {
  const allUnseen = getUnseenPosts();
  return allUnseen.slice(0, INITIAL_LOAD);
});
